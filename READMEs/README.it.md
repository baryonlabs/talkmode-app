...
> 🆕 **Latest features and install guide are in questa traduzione.** This translation covers the core docs; the visual install walkthrough (macOS 15+ Gatekeeper unblock), free lifetime license signup, Translate mode, Obsidian sync, local LLM (LM Studio / Ollama / MLX) are only on the English / Korean pages:
...
[Ultima versione](https://talkmode.baryon.ai/download/TalkMode-0.4.36.zip) → estrai → trascina in `Applications` → al primo avvio macOS potrebbe dire "da uno sviluppatore non identificato", fai clic destro → Apri una volta.
