...
> 🆕 **Latest features and install guide are in bu çeviride.** This translation covers the core docs; the visual install walkthrough (macOS 15+ Gatekeeper unblock), free lifetime license signup, Translate mode, Obsidian sync, local LLM (LM Studio / Ollama / MLX) are only on the English / Korean pages:
...
[Latest release](https://talkmode.baryon.ai/download/TalkMode-0.4.36.zip) → zip'i açın → `Applications` klasörüne sürükleyin → ilk açılışta macOS "from unknown developer" diyebilir, bir kez sağ tıklayın → Open.
