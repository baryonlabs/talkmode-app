...
> 🆕 **Latest features and install guide are in dieser Übersetzung.** This translation covers the core docs; the visual install walkthrough, free lifetime license signup, Translate mode, Obsidian sync, local LLM (LM Studio / Ollama / MLX) are only on the English / Korean pages:
...
[Neueste Version](https://talkmode.baryon.ai/download/TalkMode-0.4.36.zip) → entpacken → in `Applications` ziehen → beim ersten Start sagt macOS möglicherweise „von einem nicht identifizierten Entwickler", Rechtsklick → einmal Öffnen.
