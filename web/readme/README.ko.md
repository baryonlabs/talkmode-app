...
[최신 릴리스](https://talkmode.baryon.ai/download/TalkMode-0.4.20.zip) → 압축 해제 → `Applications`로 드래그 → 위 **3단계**를 동일하게 적용.
...
- LLM 제공자 — **기본값은 Baryon AI 호스팅(설정 불필요 · 무료 30회)**. 30회 이후에는 `설정 → LLM 제공자` 에서 Claude CLI · Codex CLI · OpenAI 키 중 하나로 전환하세요.
